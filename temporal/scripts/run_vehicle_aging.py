"""Manual trigger for the Vehicle Stock-Aging Analyst workflow (issue #32).

Run from the repository root:

    python -m temporal.scripts.run_vehicle_aging --tenant-key demo-ops-a

The script resolves the tenant key to its UUID, starts ``VehicleAgingWorkflow``
on the configured task queue, waits for the result, and prints a compact
``{scoped, recorded, deduped}`` summary alongside the full workflow result.
"""
...
async def _run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    workflow_id = args.workflow_id or (
        f"ops-vehicle-aging-manual-{tenant_id}-{datetime.now(UTC).strftime('%Y%m%d%H%M%S%f')}"
    )
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Manually trigger the Vehicle Stock-Aging Analyst workflow.",
    )
    parser.add_argument(
        "--tenant-key",
        default="demo-ops-a",
    ...
    summary = {
        "scoped": result.get("total_vehicles_scoped", 0),
