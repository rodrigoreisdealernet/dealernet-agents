...
_RATE_LIMIT_RETRY = RetryPolicy(
    maximum_attempts=5,
    non_retryable_error_types=["CoupaAuthError", "CoupaMappingError", "ValueError", "ApplicationError"],
)
...
@workflow.defn
class CoupaSyncWorkflow:
    """Orchestrates incremental polling of Coupa procurement data.

    Source-of-truth is always Coupa for all four procurement scopes:
      - requisitions   (inbound)
    ...
    For each enabled scope:
      ...
      2. Paginate through the Coupa API, persisting each page idempotently.
      3. Advance the cursor after each page so retries can resume mid-stream.

    Supports operator-triggered backfill: when mode == "backfill" the stored
    cursor is ignored and records are re-fetched from the beginning without
    advancing the durable cursor, so the live incremental resume point is
    preserved.
    """

    @workflow.run
    async def run(self, inp: CoupaSyncWorkflowInput) -> dict[str, Any]:
        ...
        scopes_to_run = inp.scopes if inp.scopes else list(coupa.COUPA_SCOPES)
        ...
        failed_scopes = [
            scope_name
            for scope_name, result in summary["scopes"].items()
            if result["status"] == "failed"
        ]
        if failed_scopes:
            raise ApplicationError(
                f"Coupa sync had {len(failed_scopes)} scope failure(s): {', '.join(failed_scopes)}",
                non_retryable=True,
            )
            ...
            while True:
                ...
                if records:
                    ...
                    persist_result = await workflow.execute_activity(
                        coupa.coupa_persist_procurement_batch,
                ...
                # During backfill, do not advance the durable cursor: backfill
                ...
                if advance_cursor is not None and mode != "backfill":
                    await workflow.execute_activity(
                        coupa.coupa_advance_sync_cursor,
