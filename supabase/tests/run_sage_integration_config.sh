...
container_name="${CONTAINER_NAME:-wynne_sage_intacct_tests_$$}"
...
echo "Running Sage Intacct integration config behavioral tests"
if ! docker exec -i "$container_name" psql -v ON_ERROR_STOP=1 -U postgres -d postgres \
      < "$repo_root"/supabase/tests/sage_integration_config.sql; then
  echo "sage_integration_config tests FAILED" >&2
  exit 1
fi

echo "sage_integration_config checks passed"
