...
container_name="${CONTAINER_NAME:-external-rental-reporting-tests-$$}"
...
echo "Running external-rental reporting assertions"
docker_psql < "$repo_root"/supabase/tests/external_rental_reporting.sql >/dev/null

echo "External-rental reporting checks passed"
