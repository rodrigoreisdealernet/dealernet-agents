...
# route exception thread + branch review bundle migration cleanly, then runs
# reset-path assertions for 20260617001500_route_exception_thread_bundle.sql.
#
# Usage (from repo root):
#   bash supabase/tests/run_route_exception_thread_bundle_reset.sh
#
# Requirements:
...
psql \
  ...
  -U postgres \
  -d postgres \
  -f "$repo_root/supabase/tests/route_exception_thread_bundle_reset.sql" \
  >/dev/null

echo "route_exception_thread_bundle reset SQL assertions passed"
echo "route_exception_thread_bundle reset checks passed"
