...
container_name="${CONTAINER_NAME:-wynne_lvl3_asset_analytics_access_tests_$$}"
...
if ! docker exec -i "$container_name" psql -v ON_ERROR_STOP=1 -U postgres -d postgres \
  < "$repo_root"/supabase/tests/asset_analytics_access_contract.sql >/dev/null; then
  echo "Asset analytics access-contract SQL test failed" >&2
  exit 1
fi

echo "Asset analytics access-contract checks passed"
