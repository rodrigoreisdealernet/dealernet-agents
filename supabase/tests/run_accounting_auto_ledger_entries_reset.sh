...
psql \
  ...
  -U postgres \
  -d postgres \
  -f "$repo_root/supabase/tests/accounting_auto_ledger_entries.sql" \
  >/dev/null

echo "Accounting auto ledger entry reset SQL assertions passed"
echo "Accounting auto ledger entry reset checks passed"
