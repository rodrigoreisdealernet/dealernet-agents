#!/usr/bin/env bash
# Validates the RLS / security-invoker behavior of the dispatch live ops views
# (migration 20260609120000_dispatch_live_ops_views.sql) in a throwaway
# Postgres container.
#
# Usage (from repo root):
#   bash supabase/tests/run_dispatch_live_ops_rls.sh
set -euo pipefail

repo_root="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"
container_name="${CONTAINER_NAME:-wynne_dispatch_rls_tests_$$}"
...
echo "Running dispatch live ops RLS behavioral tests"
if ! docker exec -i "$container_name" psql -v ON_ERROR_STOP=1 -U postgres -d postgres \
     < "$repo_root"/supabase/tests/dispatch_live_ops_rls.sql; then
  echo "Dispatch live ops RLS tests FAILED" >&2
  exit 1
fi

echo "Dispatch live ops RLS checks passed"
