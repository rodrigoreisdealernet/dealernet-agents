...
container_name="${CONTAINER_NAME:-wynne_coupa_integration_config_tests_$$}"
...
echo "Running Coupa integration config behavioral tests"
if ! docker exec -i "$container_name" psql -v ON_ERROR_STOP=1 -U postgres -d postgres \
      < "$repo_root"/supabase/tests/coupa_integration_config.sql; then
  echo "coupa_integration_config tests FAILED" >&2
  exit 1
fi

echo "coupa_integration_config checks passed"
