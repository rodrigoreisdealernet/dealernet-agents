#!/usr/bin/env bash
# Runs Descartes sync observability/recovery SQL contract tests in throwaway Postgres.
set -euo pipefail

repo_root="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"
container_name="${CONTAINER_NAME:-wynne_descartes_sync_controls_$$}"
...
echo "Running Descartes sync controls SQL tests"
if ! docker exec -i "$container_name" psql -v ON_ERROR_STOP=1 -U postgres -d postgres \
      < "$repo_root"/supabase/tests/descartes_sync_controls.sql; then
  echo "descartes_sync_controls tests FAILED" >&2
  exit 1
fi

echo "descartes_sync_controls checks passed"
