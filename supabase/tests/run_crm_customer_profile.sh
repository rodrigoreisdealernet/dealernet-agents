...
container_name="${CONTAINER_NAME:-wynne_lvl3_crm_tests_$$}"
...
echo "Running CRM customer profile smoke tests"
if ! docker exec -i "$container_name" psql -v ON_ERROR_STOP=1 -U postgres -d postgres < "$repo_root"/supabase/tests/crm_customer_profile.sql; then
  echo "CRM customer profile smoke test failed" >&2
  exit 1
fi

echo "CRM customer profile model checks passed"
