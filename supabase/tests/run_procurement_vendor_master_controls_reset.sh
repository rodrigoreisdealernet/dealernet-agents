#!/usr/bin/env bash
# Reset-path validation for procurement vendor master and purchasing
...
psql \
  ...
  -U postgres \
  -d postgres \
  -f "$repo_root/supabase/tests/procurement_vendor_master_controls_reset.sql" \
  >/dev/null

echo "procurement_vendor_master_controls reset-path validation passed"
