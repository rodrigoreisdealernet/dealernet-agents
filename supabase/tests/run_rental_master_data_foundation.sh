...
container_name="${CONTAINER_NAME:-wynne_lvl3_rental_tests_$$}"
...
echo "Running rental master data tests"
if ! docker exec -i "$container_name" psql -v ON_ERROR_STOP=1 -U postgres -d postgres < "$repo_root"/supabase/tests/rental_master_data_foundation.sql >/dev/null; then
  echo "Rental master data SQL smoke test failed" >&2
  exit 1
fi

echo "Rental master data foundation checks passed"
