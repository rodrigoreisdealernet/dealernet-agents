...
container_name="${CONTAINER_NAME:-wynne-lvl-3_org_hierarchy_tests_$$}"
...
echo "Running enterprise org hierarchy SQL checks"
if ! docker exec -i "$container_name" psql -v ON_ERROR_STOP=1 -U postgres -d postgres < "$repo_root"/supabase/tests/enterprise_org_hierarchy.sql >/dev/null; then
  echo "Enterprise org hierarchy SQL smoke test failed" >&2
  exit 1
fi

echo "Enterprise org hierarchy checks passed"
