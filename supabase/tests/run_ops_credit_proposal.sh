#!/usr/bin/env bash
# Runs credit_change_proposal RLS behavioral tests in a throwaway Postgres container.
set -euo pipefail

repo_root="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"
container_name="${CONTAINER_NAME:-wynne_credit_proposal_tests_$$}"
...
echo "Running ops_credit_proposal RLS behavioral tests"
if ! docker exec -i "$container_name" psql -v ON_ERROR_STOP=1 -U postgres -d postgres \
      < "$repo_root"/supabase/tests/ops_credit_proposal.sql; then
  echo "ops_credit_proposal tests FAILED" >&2
  exit 1
fi

echo "ops_credit_proposal checks passed"
