#!/usr/bin/env bash
# Validates compliance subject monitor read model behavior
# (migration 20260619224000_compliance_subject_monitor_inputs.sql).
set -euo pipefail
...
echo "Running compliance subject monitor behavioral tests"
if ! docker exec -i "$container_name" psql -v ON_ERROR_STOP=1 -U postgres -d postgres \
      < "$repo_root"/supabase/tests/compliance_subject_monitor_inputs.sql; then
  echo "Compliance subject monitor tests FAILED" >&2
  exit 1
fi

echo "Compliance subject monitor checks passed"
