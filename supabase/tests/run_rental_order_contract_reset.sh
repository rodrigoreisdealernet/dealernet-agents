...
# rental order/contract migration chain cleanly, then runs focused SQL
# assertions for the 20251210000000_rental_order_contract.sql objects.
set -euo pipefail
...
psql \
  ...
  -U postgres \
  -d postgres \
  -f "$repo_root/supabase/tests/rental_order_contract_reset.sql" \
  >/dev/null

echo "Rental order/contract reset checks passed"
