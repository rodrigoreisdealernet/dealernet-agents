...
psql \
  ...
  -U postgres \
  -d postgres \
  -f "$repo_root/supabase/tests/driver_dispatch_execution_reset.sql" \
  >/dev/null

echo "Driver dispatch execution reset SQL assertions passed"
echo "Driver dispatch execution reset checks passed"
