#!/usr/bin/env bash
# Runs quote_fee_engine behavioral tests in a throwaway Postgres container.
#
# Usage (from repo root):
#   bash supabase/tests/run_quote_fee_engine.sh
set -euo pipefail

repo_root="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"
container_name="${CONTAINER_NAME:-wynne_quote_fee_engine_tests_$$}"
...
echo "Running quote fee engine + tax preset behavioral tests"
if ! docker exec -i "$container_name" psql -v ON_ERROR_STOP=1 -U postgres -d postgres \
     < "$repo_root"/supabase/tests/quote_fee_engine.sql; then
  echo "quote_fee_engine tests FAILED" >&2
  exit 1
fi

echo "quote_fee_engine checks passed"
