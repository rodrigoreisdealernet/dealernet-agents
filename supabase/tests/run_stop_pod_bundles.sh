#!/usr/bin/env bash
# Runs stop_pod_bundles behavioral tests in a throwaway Postgres container.
set -euo pipefail

repo_root="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"
container_name="${CONTAINER_NAME:-wynne_pod_bundles_tests_$$}"
...
echo "Running stop_pod_bundles behavioral tests"
if ! docker exec -i "$container_name" psql -v ON_ERROR_STOP=1 -U postgres -d postgres \
      < "$repo_root"/supabase/tests/stop_pod_bundles.sql; then
  echo "stop_pod_bundles tests FAILED" >&2
  exit 1
fi

echo "stop_pod_bundles checks passed"
