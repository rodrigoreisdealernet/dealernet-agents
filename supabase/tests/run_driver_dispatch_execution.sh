#!/usr/bin/env bash
# Runs driver_dispatch_execution behavioral tests in a throwaway Postgres container.
set -euo pipefail

repo_root="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"
container_name="${CONTAINER_NAME:-wynne_dispatch_exec_tests_$$}"
...
echo "Running driver_dispatch_execution behavioral tests"
if ! docker exec -i "$container_name" psql -v ON_ERROR_STOP=1 -U postgres -d postgres \
      < "$repo_root"/supabase/tests/driver_dispatch_execution.sql; then
  echo "driver_dispatch_execution tests FAILED" >&2
  exit 1
fi

echo "driver_dispatch_execution checks passed"
