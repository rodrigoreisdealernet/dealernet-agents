...
# project equipment hire workflow migration cleanly, then runs reset-path
# assertions for 20260614160000_project_equipment_hire_workflow.sql.
set -euo pipefail
...
psql \
  ...
  -U postgres \
  -d postgres \
  -f "$repo_root/supabase/tests/project_equipment_hire_workflow_reset.sql" \
  >/dev/null

echo "Project equipment hire workflow reset checks passed"
