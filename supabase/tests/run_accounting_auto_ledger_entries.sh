...
container_name="${CONTAINER_NAME:-wynne_lvl3_accounting_ledger_tests_$$}"
...
echo "Running accounting auto ledger entry SQL checks"
docker exec -i "$container_name" psql -v ON_ERROR_STOP=1 -U postgres -d postgres < "$repo_root"/supabase/tests/accounting_auto_ledger_entries.sql

echo "Accounting auto ledger entry checks passed"
