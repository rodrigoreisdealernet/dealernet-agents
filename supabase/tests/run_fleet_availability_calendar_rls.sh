...
#   bash supabase/tests/run_fleet_availability_calendar_rls.sh
set -euo pipefail

repo_root="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"
container_name="${CONTAINER_NAME:-wynne_fleet_cal_rls_tests_$$}"
...
echo "Running fleet availability calendar RLS behavioral tests"
if ! docker exec -i "$container_name" psql -v ON_ERROR_STOP=1 -U postgres -d postgres \
     < "$repo_root"/supabase/tests/fleet_availability_calendar_rls.sql; then
  echo "Fleet availability calendar RLS tests FAILED" >&2
  exit 1
fi

echo "Fleet availability calendar RLS checks passed"
