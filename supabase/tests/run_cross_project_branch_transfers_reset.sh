...
# cross-project/branch transfer migration cleanly, then runs reset-path
# assertions for 20260613170000_cross_project_branch_transfers.sql.
set -euo pipefail
...
psql \
  ...
  -U postgres \
  -d postgres \
  -f "$repo_root/supabase/tests/cross_project_branch_transfers_reset.sql" \
  >/dev/null

echo "Cross-project branch transfers reset checks passed"
