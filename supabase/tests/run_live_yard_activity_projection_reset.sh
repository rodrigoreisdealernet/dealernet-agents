...
# live-yard projection migration chain cleanly, then runs reset-path assertions
# for 20260613150000_live_yard_activity_projection.sql against the rebuilt DB.
set -euo pipefail
...
psql \
  ...
  -U postgres \
  -d postgres \
  -f "$repo_root/supabase/tests/live_yard_activity_projection_reset.sql" \
  >/dev/null

echo "Live yard activity projection reset checks passed"
