#!/usr/bin/env bash
# RLS / role-gating behavioral tests for the SmartEquip delivery observability surface
...
# executes smartequip_delivery_observability.sql to confirm:
...
#   - smartequip_quarantine_exchange RPC: admin happy path, read_only denied, cross-tenant denied
#   - smartequip_mark_replayed RPC: admin happy path, read_only denied
#   - smartequip_disable_exchange_retry RPC: admin wrapper path for disabling retries
#   - service_role policy effective: sees rows from all tenants
#
# Usage (from repo root):
#   bash supabase/tests/run_smartequip_delivery_observability.sh
set -euo pipefail
...
echo "Running SmartEquip delivery observability smoke tests"
if ! docker exec -i "$container_name" psql -v ON_ERROR_STOP=1 -U postgres -d postgres \
      < "$repo_root"/supabase/tests/smartequip_delivery_observability.sql; then
  echo "smartequip_delivery_observability tests FAILED" >&2
  exit 1
fi

echo "smartequip_delivery_observability checks passed"
