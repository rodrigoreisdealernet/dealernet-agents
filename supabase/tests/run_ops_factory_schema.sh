#!/usr/bin/env bash
# Runs Ops Factory persistence migration smoke tests in a throwaway Postgres container.
set -euo pipefail

repo_root="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"
container_name="${CONTAINER_NAME:-wynne_ops_factory_tests_$$}"
...
echo "Running ops_factory_schema smoke tests"
if ! docker exec -i "$container_name" psql -v ON_ERROR_STOP=1 -U postgres -d postgres \
      < "$repo_root"/supabase/tests/ops_factory_schema.sql; then
  echo "ops_factory_schema smoke tests FAILED" >&2
  exit 1
fi

echo "ops_factory_schema checks passed"
