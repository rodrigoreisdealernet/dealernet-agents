#!/usr/bin/env bash
# Validates the access-control behavior of portal_get_contract_schedule
# (migration 20260610120000_fix_seed_entity_types_and_schedule_join.sql) in a
# throwaway Postgres container.
#
# Usage (from repo root):
#   bash supabase/tests/run_portal_schedule_rls.sh
set -euo pipefail

repo_root="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"
container_name="${CONTAINER_NAME:-wynne_portal_schedule_rls_tests_$$}"
...
echo "Running portal schedule RLS behavioral tests"
if ! docker exec -i "$container_name" psql -v ON_ERROR_STOP=1 -U postgres -d postgres \
     < "$repo_root"/supabase/tests/portal_schedule_rls.sql; then
  echo "Portal schedule RLS tests FAILED" >&2
  exit 1
fi

echo "Portal schedule RLS checks passed"
