...
psql \
  ...
  -U postgres \
  -d postgres \
  -f "$repo_root/supabase/tests/driver_dvir_and_exceptions_reset.sql" \
  >/dev/null

echo "driver_dvir_and_exceptions reset SQL assertions passed"
echo "driver_dvir_and_exceptions reset checks passed"
