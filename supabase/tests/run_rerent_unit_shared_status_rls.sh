#!/usr/bin/env bash
# Validates the RLS / security-invoker / GRANT behavior of
# rerent_unit_status_log and v_rerent_unit_current_status
# (migration 20260613140000_rerent_unit_shared_status.sql)
# in a throwaway Postgres container.
#
# Usage (from repo root):
#   bash supabase/tests/run_rerent_unit_shared_status_rls.sh
set -euo pipefail

repo_root="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"
container_name="${CONTAINER_NAME:-wynne_rerent_status_rls_tests_$$}"
...
echo "Running re-rented unit shared status RLS behavioral tests"
if ! docker exec -i "$container_name" psql -v ON_ERROR_STOP=1 -U postgres -d postgres \
     < "$repo_root"/supabase/tests/rerent_unit_shared_status_rls.sql; then
  echo "Re-rented unit shared status RLS tests FAILED" >&2
  exit 1
fi

echo "Re-rented unit shared status RLS checks passed"
