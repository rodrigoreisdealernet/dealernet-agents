#!/usr/bin/env bash
# Runs driver_runsheet_contact_fields behavioral tests in a throwaway Postgres container.
set -euo pipefail

repo_root="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"
container_name="${CONTAINER_NAME:-wynne_runsheet_contact_tests_$$}"
...
echo "Running driver_runsheet_contact_fields behavioral tests"
if ! docker exec -i "$container_name" psql -v ON_ERROR_STOP=1 -U postgres -d postgres \
      < "$repo_root"/supabase/tests/driver_runsheet_contact_fields.sql; then
  echo "driver_runsheet_contact_fields tests FAILED" >&2
  exit 1
fi

echo "driver_runsheet_contact_fields checks passed"
