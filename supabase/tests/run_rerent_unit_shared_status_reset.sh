#!/usr/bin/env bash
# Validates the 20260613140000_rerent_unit_shared_status.sql migration via a
# full supabase db reset, then runs the behavioral RLS tests against the
# resulting database.
#
# Usage (from repo root):
#   bash supabase/tests/run_rerent_unit_shared_status_reset.sh
#
# Requirements:
...
psql \
  ...
  -U postgres \
  -d postgres \
  -f "$repo_root/supabase/tests/rerent_unit_shared_status_rls.sql" \
  >/dev/null

echo "Re-rented unit shared status reset checks passed"
