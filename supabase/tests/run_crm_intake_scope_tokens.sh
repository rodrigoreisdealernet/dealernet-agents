...
container_name="${CONTAINER_NAME:-wynne_lvl3_intake_tests_$$}"
...
echo "Running CRM intake scope token smoke tests"
if ! docker exec -i "$container_name" psql -v ON_ERROR_STOP=1 -U postgres -d postgres < "$repo_root"/supabase/tests/crm_intake_scope_tokens.sql; then
  echo "CRM intake scope token smoke test failed" >&2
  exit 1
fi

echo "CRM intake scope token checks passed"
