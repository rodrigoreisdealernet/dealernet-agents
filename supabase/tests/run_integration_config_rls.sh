...
container_name="${CONTAINER_NAME:-wynne_integration_config_rls_tests_$$}"
...
echo "Running integration_config RLS behavioral tests"
if ! docker exec -i "$container_name" psql -v ON_ERROR_STOP=1 -U postgres -d postgres \
     < "$repo_root"/supabase/tests/integration_config_rls.sql; then
  echo "integration_config RLS tests FAILED" >&2
  exit 1
fi

echo "integration_config RLS checks passed"
