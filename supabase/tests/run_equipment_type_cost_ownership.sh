...
container_name="${CONTAINER_NAME:-wynne_lvl3_cost_ownership_tests_$$}"
...
echo "Running equipment-type cost-of-ownership and profitability tests"
if ! docker exec -i "$container_name" psql -v ON_ERROR_STOP=1 -U postgres -d postgres \
  < "$repo_root"/supabase/tests/equipment_type_cost_ownership.sql; then
  echo "Equipment-type cost-of-ownership SQL test failed" >&2
  exit 1
fi

echo "Equipment-type cost-of-ownership checks passed"
