-- Fix two CI seed failures:
--
-- 1. seed.sql uses entity types note, document, transfer, rate_card and
--    relationship types customer_has_note / customer_has_document that were
--    absent from rental_entity_type_catalog / rental_relationship_type_catalog.
--    This caused rental_assert_entity_type to raise on every seed run.
--
-- 2. portal_get_contract_schedule joined v_current_assets on
--    "a.asset_id = l.asset_id" where a.asset_id is uuid (entities.id) but
--    l.asset_id is text (ev.data->>'asset_id').  Postgres rejects the
--    implicit uuid = text comparison with "operator does not exist".

-- ---------------------------------------------------------------------------
-- 1. Extend rental_entity_type_catalog with all entity types used by seed.sql
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- 2. Extend rental_relationship_type_catalog with relationship types used by
--    seed.sql (customer_has_note, customer_has_document).
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- 3. Fix portal_get_contract_schedule: cast l.asset_id (text) to uuid so it
--    matches v_current_assets.asset_id (uuid) in the LEFT JOIN predicate.
-- ---------------------------------------------------------------------------
