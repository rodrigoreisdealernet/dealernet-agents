-- Fix the write-RPC role guard (create_entity_with_version,
-- rental_upsert_entity_current_state, rental_upsert_relationship).
--
-- Two bugs are fixed together:
--  1) The guard read ONLY the legacy GUC current_setting('request.jwt.claim.role').
--     Modern PostgREST puts the role in the request.jwt.claims JSON, so that legacy
--     GUC is empty for API callers -> every API role read as '' .
--  2) 20260607140000 then allowed v_request_role in ('', 'service_role'), so with (1)
--     ALL API callers (incl. anon / read_only) could write — a security hole the
--     access-contract tests catch (authenticated read_only / no-claim must be denied).
--
-- Fix: read the role from the modern claims JSON, falling back to the legacy GUC
-- (which the seed/migrations set explicitly to 'service_role'), and require
-- service_role | authenticated+write-role. No '' allowance. Regenerates the 3
-- functions identically to 20260607140000 with only the role-read + guard changed.
