-- Enforce canonical quote/order/contract boundary for reservation conversion.
-- Draft orders must be quoted/approved before conversion to reservation contract.
  ...
  loop
    perform create_entity_with_version(
      ...
      p_data => jsonb_build_object(
        ...
        'fulfillment_branch_id', coalesce(nullif(v_line.branch_id, '')::uuid, nullif(v_order_data->>'branch_id', '')::uuid),
...
revoke all on function public.rental_convert_quote_to_reservation(uuid) from public;
