-- Provide a portal-only financial read surface that enforces customer/account/job-site/contract
-- scope inside the database so unauthorized rows never leave PostgREST.
...
begin
  ...
  authorized_contracts as (
    ...
    where current_entity_rows.entity_type = 'rental_contract'
      and (
        -- service_role is reserved for trusted backend/CI callers and may inspect all rows without
        -- customer portal claim narrowing; browser clients only reach the authenticated path below.
