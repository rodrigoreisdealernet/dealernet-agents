...
import {
  applySmartEquipDeliveryUpdate,
  createSmartEquipDeliveryState,
  diagnoseSmartEquipFailure,
  toOperatorDeliveryView,
  type SmartEquipDeliveryFailure,
  type SmartEquipExchangeSnapshot,
} from '@/lib/smartequip-delivery-contract';

function buildSnapshot(
  overrides: Partial<SmartEquipExchangeSnapshot> = {},
): SmartEquipExchangeSnapshot {
  return {
...
function buildFailure(
  overrides: Partial<SmartEquipDeliveryFailure> = {},
): SmartEquipDeliveryFailure {
  return {
    failureClass: 'auth',
...
describe('applySmartEquipDeliveryUpdate — event emission', () => {
  it('applies a delivered snapshot and resets retry state', () => {
    const state = createSmartEquipDeliveryState();
    const result = applySmartEquipDeliveryUpdate(state, {
      dedupeKey: 'evt-1',
...
describe('applySmartEquipDeliveryUpdate — dedupe', () => {
  it('dedupes repeated updates by dedupe key', () => {
    const first = applySmartEquipDeliveryUpdate(createSmartEquipDeliveryState(), {
      dedupeKey: 'evt-1',
      snapshot: buildSnapshot(),
    });
    const second = applySmartEquipDeliveryUpdate(first.state, {
      dedupeKey: 'evt-1',
...
describe('applySmartEquipDeliveryUpdate — retry exhaustion → dead letter', () => {
  it('dead-letters when retries are exhausted', () => {
    const base = applySmartEquipDeliveryUpdate(createSmartEquipDeliveryState(), {
      dedupeKey: 'evt-1',
      snapshot: buildSnapshot({ deliveryStatus: 'retrying', retryCount: 3 }),
    });
    const exhausted = applySmartEquipDeliveryUpdate(
  ...
  it('preserves latest snapshot when transitioning to dead_lettered', () => {
    const withSnapshot = applySmartEquipDeliveryUpdate(createSmartEquipDeliveryState(), {
      dedupeKey: 'evt-1',
      snapshot: buildSnapshot({ deliveryStatus: 'retrying' }),
    });
    const exhausted = applySmartEquipDeliveryUpdate(
...
describe('applySmartEquipDeliveryUpdate — quarantine and replay', () => {
  it('reflects quarantined status from snapshot', () => {
    const result = applySmartEquipDeliveryUpdate(createSmartEquipDeliveryState(), {
      dedupeKey: 'evt-1',
...
describe('toOperatorDeliveryView', () => {
  it('returns ok severity for a delivered exchange', () => {
    const state = applySmartEquipDeliveryUpdate(createSmartEquipDeliveryState(), {
      dedupeKey: 'evt-1',
      snapshot: buildSnapshot({ deliveryStatus: 'delivered' }),
    }).state;
  ...
  it('returns identical views for equivalent snapshots regardless of source system', () => {
    const stateA = applySmartEquipDeliveryUpdate(createSmartEquipDeliveryState(), {
      dedupeKey: 'evt-1',
      snapshot: buildSnapshot({ sourceSystem: 'smartequip-prod', deliveryStatus: 'delivered' }),
    }).state;
    const stateB = applySmartEquipDeliveryUpdate(createSmartEquipDeliveryState(), {
      dedupeKey: 'evt-1',
      snapshot: buildSnapshot({ sourceSystem: 'smartequip-staging', deliveryStatus: 'delivered' }),
    }).state;
...
describe('diagnoseSmartEquipFailure', () => {
  it('provides auth diagnosis', () => {
    expect(diagnoseSmartEquipFailure('auth')).toContain('OAuth');
  });

  it('provides signature diagnosis', () => {
    expect(diagnoseSmartEquipFailure('signature')).toContain('HMAC');
  });

  it('provides mapping diagnosis', () => {
    expect(diagnoseSmartEquipFailure('mapping')).toContain('transform');
  });

  it('provides provider_policy diagnosis', () => {
    expect(diagnoseSmartEquipFailure('provider_policy')).toContain('rate limits');
  });

  it('provides fallback diagnosis for unknown', () => {
    expect(diagnoseSmartEquipFailure('unknown')).toContain('connector logs');
  });

  it('provides fallback diagnosis for null', () => {
    expect(diagnoseSmartEquipFailure(null)).toContain('connector logs');
  });
});
