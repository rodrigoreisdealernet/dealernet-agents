...
are provided. Nothing here touches other projects' existing `dev`/`test`/`prod`
...
| `deploy/k8s/namespaces.yaml` | `dia-dev`, `dia-test` namespaces |
| `deploy/k8s/rbac-nonprod.yaml` | Per-namespace `gha-deployer` SA + namespace-scoped Role/RoleBinding |
| `deploy/k8s/rbac-dev-db-bootstrap.yaml` | `dia-supabase` bootstrap SAs + least-privilege Roles/RoleBindings for GitHub bootstrap and in-cluster DB exec |
| `.github/workflows/deploy-dev.yml` | Gated deploy to `dia-dev`; runs Supabase migrations + demo-baseline seed/assertions via in-cluster bootstrap Job |

## Go-live checklist (your infra/admin steps)

Live `dia-dev` deployment turns on when **all** of these are done:
...
2. **Cluster namespaces + RBAC** — against a cluster that is **not** another project's:
   ```bash
...
3. **Self-hosted deploy runner** — register a runner with labels
   `self-hosted, linux, x64, factory-deploy-nonprod`, reachable to `dia-dev`/`dia-test`
...
5. **Enable the gate** — set repo **variables**:
   ...
   - `DIA_DEV_NAMESPACE = dia-dev`
   - `DIA_DB_BOOTSTRAP_USER = <operator-supplied least-privilege bootstrap role>`
   - `DIA_DB_BOOTSTRAP_DB_NAME = <operator-supplied bootstrap target database>`
...
6. **Dev app deploy secret** — add repo **secret** `KUBE_CONFIG_DEV` containing the
   namespace-scoped `gha-deployer` kubeconfig for `dia-dev` (Helm app deploy only).
7. **Dev DB bootstrap secret** — add repo **secret** `KUBE_CONFIG_DEV_DB_BOOTSTRAP`
   ...
   bootstrap Job/ConfigMap lifecycle + bootstrap job pod log reads in `dia-supabase`
   (it no longer executes directly in the DB pod from GitHub Actions).

   Example (AKS/admin context):
   ```bash
   kubectl -n dia-supabase create token gha-db-bootstrap > /tmp/gha-db-bootstrap.token
   kubectl config set-cluster dia \
   ...
   kubectl config set-context gha-db-bootstrap@dia-supabase \
     --cluster=dia --user=gha-db-bootstrap --namespace=dia-supabase \
     --kubeconfig /tmp/kubeconfig-db-bootstrap
   kubectl config use-context gha-db-bootstrap@dia-supabase --kubeconfig /tmp/kubeconfig-db-bootstrap
   ```
   ...
   gh variable set DIA_DB_BOOTSTRAP_USER --body '<least-privilege-bootstrap-role>'
   gh variable set DIA_DB_BOOTSTRAP_DB_NAME --body '<bootstrap-target-database>'
   ```

`deploy-dev.yml` now uses in-cluster service account `dia-db-bootstrap` (from
`deploy/k8s/rbac-dev-db-bootstrap.yaml`) for DB-pod `exec` inside `dia-supabase`;
