...
test('AC2: TopBar renderiza seletor de idioma pt-BR/en-US que chama setLocale', () => {
  ...
  assert.match(src, /<LocaleFlag\s+locale=\{locale\}\s*\/>/, 'trigger deve exibir a bandeira do idioma atual')
  assert.doesNotMatch(src, /<Languages\b/, 'trigger nao deve exibir o icone <Languages> (issue #129)')
  assert.doesNotMatch(src, /\{locale\}<\/span>/, 'trigger nao deve exibir o codigo do locale (pt-BR/en-US) (issue #129)')
