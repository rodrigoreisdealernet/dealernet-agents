# MuleSoft observability, delivery diagnostics, and operator recovery controls

Story: **MuleSoft observability and recovery controls** (issue #1151).
Depends on: epic #892 (shared sync event / recovery primitives), #485 (MuleSoft iPaaS archetype).

---

## Purpose

Define the shared observability surface and operator recovery model for MuleSoft API exchange
...
Two new tables in `supabase/migrations/20260611085000_mulesoft_delivery_observability.sql`:
...
| Column | Purpose |
|---|---|
| `delivery_event_id` | FK back to `mulesoft_delivery_events` |
...
Two security-definer RPCs are available to `admin` and `branch_manager` roles:

### `mulesoft_quarantine_exchange(p_delivery_event_id, p_quarantine_reason, p_replay_eligible, p_operator_notes)`

- Marks the delivery event as `quarantined`.
- Inserts a row into `mulesoft_dead_letter_queue`.
- Use when retries are not appropriate and the exchange needs investigation.
- Returns the DLQ row ID.

### `mulesoft_mark_replayed(p_dlq_id, p_replay_actor, p_operator_notes)`

- Requires `replay_eligible = true` on the DLQ row.
- Inserts a new `mulesoft_delivery_events` row with `delivery_status = 'replayed'` and `replayed_from_id` pointing to the original event.
...
| View | Purpose |
|---|---|
| `v_mulesoft_delivery_dashboard` | Per-flow, per-direction delivery health summary with counts by status and failure class |
| `v_mulesoft_failed_exchanges` | Operator-facing list of exchanges needing attention (retrying / dead_lettered / quarantined) |
| `v_mulesoft_reconciliation_summary` | Per-flow, per-day delivery and failure counts for gap detection |
...
`frontend/src/lib/mulesoft-delivery-contract.ts` provides:

- `MulesoftDeliveryContractState` — client-side delivery state including status, retry count, failure, and DLQ flags.
- `applyMulesoftDeliveryUpdate(...)` — dedupes by `dedupeKey`, tracks retry/dead-letter transitions.
...
- `diagnoseMulesoftFailure(failureClass)` — returns human-readable investigation guidance for each failure class.

Tests: `frontend/src/test/mulesoft-delivery-contract.test.ts`
...
4. Call `mulesoft_mark_replayed(...)` for each affected entry.
...
4. If the window was long (> a few hours), reconcile against `v_mulesoft_reconciliation_summary`
...
4. After deploying the fix, call `mulesoft_mark_replayed(...)` for eligible DLQ entries.
...
1. Check `v_mulesoft_delivery_dashboard` for the affected flow's `provider_policy_failure_count`
...
4. Check `v_mulesoft_reconciliation_summary` to confirm the volume of affected exchanges is
...
Use `v_mulesoft_reconciliation_summary` to detect drift:
...
select
  ...
  failure_count
from v_mulesoft_reconciliation_summary
...
| Layer | Tests |
|---|---|
| Frontend contract | `frontend/src/test/mulesoft-delivery-contract.test.ts` |
| Supabase schema / RLS | `supabase/tests/mulesoft_delivery_observability.sql` |
| Supabase schema runner | `supabase/tests/run_mulesoft_delivery_observability.sh` |
...
MuleSoft connector adapter delivered under issue #485, which will emit delivery events into
`mulesoft_delivery_events` and call the operator RPCs from its activity layer.
