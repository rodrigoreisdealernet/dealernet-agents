...
- Supabase Studio dashboard credentials (`supabase-dashboard` in `wynne-supabase`)
...
|---|---|---|---|---|
| dev | `wynne/dev/runtime` | `anon-key` | `frontend-secrets-wynne-dev` | `VITE_SUPABASE_ANON_KEY` |
| dev | `wynne/dev/runtime` | `service-role-key` | `temporal-worker-secrets-wynne-dev` | `SUPABASE_SERVICE_ROLE_KEY` |
| dev | `wynne/dev/acr-pull` | `dockerconfigjson` | `acr-pull` ¹ | `.dockerconfigjson` |
| test | `wynne/test/runtime` | `anon-key` | `frontend-secrets-wynne-test` | `VITE_SUPABASE_ANON_KEY` |
| test | `wynne/test/runtime` | `service-role-key` | `temporal-worker-secrets-wynne-test` | `SUPABASE_SERVICE_ROLE_KEY` |
| prod | `wynne/prod/runtime` | `anon-key` | `frontend-secrets-wynne-prod` | `VITE_SUPABASE_ANON_KEY` |
| prod | `wynne/prod/runtime` | `service-role-key` | `temporal-worker-secrets-wynne-prod` | `SUPABASE_SERVICE_ROLE_KEY` |
...
1. **Write a new secret version** in OpenBao (KV-v2 versions each write automatically, so
   ...
   ```bash
   bao kv patch secret/wynne/dev/runtime anon-key="$NEW_ANON_KEY"
   ```
...
3. **Verify** the Kubernetes Secret has updated:
   ```bash
   kubectl get secret frontend-secrets-wynne-dev -n wynne-dev \
...
4. **Restart affected pods** if the app does not watch for secret changes at runtime:
   ...
   ```
5. **Destroy the old KV-v2 version** (`bao kv destroy -versions=<n> secret/wynne/dev/runtime`)
...
```bash
# dev uses a namespaced SecretStore (openbao-dev in wynne-dev)
kubectl -n wynne-dev get secretstore openbao-dev \
  -o jsonpath='{.status.conditions[*].type}={.status.conditions[*].status}{"\n"}'   # want Ready=True
kubectl -n wynne-vault get deploy openbao        # OpenBao up
...
1. Bootstrap OpenBao and provision secret values (see `deploy/k8s/wynne-vault/openbao-dev.yaml`
   bootstrap comments and the path/field table above).
2. Apply the `SecretStore` and `eso-vault-auth` SA:
   ```bash
   kubectl apply -f deploy/k8s/wynne-dev/secretstore-openbao.yaml
   ```
3. Verify the `SecretStore` reports `Ready=True`.
4. Set `externalSecrets.enabled: true` in the environment values file and deploy:
   ```bash
   helm upgrade --install app-dev charts/app -n wynne-dev \
     -f charts/app/values-dev.yaml \
     --set externalSecrets.enabled=true
   ```
5. ESO syncs and takes ownership of the Kubernetes Secrets. Verify:
   ```bash
   kubectl get externalsecrets -n wynne-dev
   kubectl get secret frontend-secrets-wynne-dev -n wynne-dev -o yaml | grep -A2 ownerReferences
   ```
...
| Supabase dashboard credentials (`supabase-dashboard` secret) | Platform + maintainer approver | Kubernetes secret in `wynne-supabase` sourced from approved secret manager path | Break-glass Studio access only | Every 30 days and after each break-glass use |
...
- `WYNNE_DB_BOOTSTRAP_USER = <least-privilege bootstrap role>`
- `WYNNE_DB_BOOTSTRAP_DB_NAME = <bootstrap target database>`
...
2. **Update secret references**
   ...
   - For DB bootstrap rotations, update `KUBE_CONFIG_DEV_DB_BOOTSTRAP` and confirm
     the paired `WYNNE_DB_BOOTSTRAP_USER` and `WYNNE_DB_BOOTSTRAP_DB_NAME` values
...
1. **Demo app sign-in**
   - `admin@wynne-rental.dev` and `operator@wynne-rental.dev` succeed with newly issued credentials.
...
5. Re-disable any temporary legacy account access (including `demo@wynne-rental.dev`) after use.
