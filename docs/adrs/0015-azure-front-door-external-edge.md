...
## Decision
We front the AKS workloads with **Azure Front Door (Standard/Premium)**. The `dia-afd` profile (`rg-dia-dev`) terminates TLS at the edge and forwards to AKS LoadBalancer origins. Two endpoints:
- `dia-app` → origin group `app-og` → `20.161.209.34` (frontend LB)
- `dia-api` → origin group `api-og` → `20.36.251.229` (Supabase/Kong LB)
...
## Evidence
- Live: `az afd profile/endpoint/origin/route list` — profile `dia-afd` (`rg-dia-dev`, FrontDoorId `e9bd4dea-…`); endpoints `dia-app` (`dia-app-a4bde4gwecdnfpfb.a02.azurefd.net`), `dia-api` (`dia-api-fvd0fcfubfb2drcy.a02.azurefd.net`); origins `20.161.209.34`/`20.36.251.229` (`http=80 https=443`); routes `/*`, https-redirect Enabled, forwarding `HttpOnly`
