...
- `aks-selfheal-staging` (`rg-selfheal-staging`) — hosts the wynne **dev/test** workloads (namespaces `wynne-dev`, `wynne-test`). Node RG `MC_rg-selfheal-staging_aks-selfheal-staging_eastus2` holds the app/api LoadBalancer public IPs.
- `aks-selfheal-prod` (`rg-selfheal-prod`) — intended **prod** cluster (`wynne-prod`).
...
**Edge (Front Door / ADR-0015):** `wynne-afd` (`rg-wynne-dev`); siblings `selfheal-afd`, `selfheal-prod-afd`, `equip-afd`.

**Runners:** `github-runners-rg` (self-hosted runner infra, ADR-0008).

## Consequences
- Naming is cross-cutting: "wynne" lives across `rg-wynne-dev` (Front Door), `rg-selfheal-staging` (cluster/ACR), and the `wynne-*` namespaces — reviewers must not assume one resource group holds everything.
