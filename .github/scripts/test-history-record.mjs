...
// Vitest `json` reporter: { numPassedTests, numFailedTests, numPendingTests,
...
// `coverage` and `quality` are not pass/fail suites — each carries its compute-script
// object verbatim under `kind:"<format>"`. Renderers chart them separately and exclude
// them from the pass-rate suites. Short-circuit before the stats/tests machinery below.
if (format === 'coverage' || format === 'quality') {
  const sha2 = process.env.GITHUB_SHA || null;
  const serverUrl2 = process.env.GITHUB_SERVER_URL || 'https://github.com';
  const repo2 = process.env.GITHUB_REPOSITORY || '';
  const runId2 = process.env.GITHUB_RUN_ID || null;
  const rec = {
    ts: process.env.RUN_TS || new Date().toISOString(),
    suite: format,
    kind: format,
    outcome: report == null ? 'error' : 'passed',
    [format]: report || {},
    run_id: runId2,
    run_number: process.env.GITHUB_RUN_NUMBER || null,
    run_url: runId2 && repo2 ? `${serverUrl2}/${repo2}/actions/runs/${runId2}` : null,
    sha: sha2,
    sha_short: sha2 ? sha2.slice(0, 7) : null,
