...
If code-level follow-up is needed, assign Copilot via GraphQL (the only method that works — `gh issue edit --add-assignee` does NOT work for the bot):
```bash
