...
**1. Standing posture sweep (every run) — consult the Architecture Audit.**
```bash
...
```bash
python scripts/audit/check_workflow_security.py      # pull_request_target+secrets, write-all
python scripts/audit/check_view_security_invoker.py  # views that bypass RLS
```
...
- **Workflow permissions:** minimal `permissions:`, no unsafe `pull_request_target` pattern. (Do NOT require a human/maintainer gate on workflow edits — the factory merges control-plane changes autonomously once you've reviewed them; reach a terminal verdict in-lane.)
