...
4. For `queue:database` **issues**, if a design decision is missing, route to `queue:architecture` and state the exact missing decision.
5. For **PRs**, never add `needs-design` or `queue:architecture` (Factory Architect does not service PRs). Always decide in this lane: approve when safe, or request changes with the exact missing decision/fix.
...
- If design context is missing on a PR, request changes with the exact missing decision and keep `needs-database-review` — do not reroute the PR to architecture labels.
